        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    vutils.save_image(sampled_imgs.permute(0,3,1,2), os.path.join("results", f"transformer_{epoch}.jpg"), nrow=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint-path', type=str, default='last.ckpt', help='Path to checkpoint.')
    ...
    parser.add_argument('--num-workers', type=float, default=1, help='number of workers for dataloader.')

    parser.add_argument('--pkeep', type=float, default=0.5, help='Percentage for how much latent codes to keep.')
    parser.add_argument('--sos-token', type=int, default=0, help='Start of Sentence token.')

    args = parser.parse_args()
    args.dataset_path = r"flowers"
    args.checkpoint_path = r"last.ckpt"
